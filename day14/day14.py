...
def segment_to_points(a: Point, b: Point) -> list[Point]:
    ...
    if a.x == b.x:
        ...
        return [Point(a.x, y) for y in range(start, end + 1)]
    elif a.y == b.y:
        start, end = min(a.x, b.x), max(a.x, b.x)
        return [Point(x, a.y) for x in range(start, end + 1)]
    else:
...
def draw_cave_canvas(rock_points: set[Point], draw_floor: bool) -> Canvas:
    ...
    return [
        [
            ROCK if (x, y) in rock_points or (draw_floor and y == max_y + 2) else AIR
