...
def row_coverage_by_sensor(y_row: int, sensor: Sensor) -> Interval:
    ...
    return Interval(
        left=sensor.pos.x - row_reach,
        right=sensor.pos.x + row_reach
    )
...
def points_covered_in_row(y_row: int, sensors: list[Sensor]) -> set[Point]:
    return reduce(
        set.union,
        map(partial(row_coverage_by_sensor_points, y_row), tqdm(sensors))
    )
...
def subtract_interval(a: Interval, b: Interval) -> list[Interval]:
    ...
    if b.left > a.left and b.right < a.right:   # [..a...[.b..]..] -> [..a..].....[a]
        return [
            Interval(a.left, b.left - 1), 
            Interval(b.right + 1, a.right)
        ]
    if b.left <= a.left:                        # [..b..]...   
